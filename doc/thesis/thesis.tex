...
\begin{enumerate}
    ...
    \item \emph{Kdybych nedomlouval datum soustředění ale barvu závěsů do klubovny, na které se všichni shodnou alespoň většinově, co použiji?} --
        Jelikož je z hlediska výběru vhodného data poměrně populární služba Doodle, dá se očekávat, že ovlivní odpovědi na předešlou otázku. Proto na ni tato navazuje a záměrně nastiňuje situaci, v níž Doodle použít nelze.
    ...
    \item \emph{Zařizuji kancelář, ve které mají být telefony. Kde se to pokusím domluvit?} --
        Tento scénář se zaměřuje na povědomí lidí o multimediální CMC mimo službu Skype. Jako příklad zde slouží VoIP telefonie. Ani zde účastníci dotazníku nedostali možnost doplnit vlastní odpověď.
\end{enumerate}
...
\subsubsection*{\textbf{4. otázka:} \emph{Organizuji malou veřejnou událost, např. minikonferenci s přednáškami nebo místní mistrovství v házení šipek. Co použiji pro propagaci a evidenci?}}
...
\section{Realizovatelnost sjednocujícího systému}
...
asynchronní zpráva (jednomu, mnoha)

synchronní zpráva (jednomu, mnoha)
...
\section{Praktické možnosti propojení}\label{practicalOptions}
...
e-mail (gmail) + facebook messages

skupinový e-mail + google groups + skupinová facebook message, facebook groups (group mail)

jabber + gtalk + facebook chat + (icq, aim, windows live, yahoo, myspace, gadu-gadu, skype chat, ...)

jabber conference + gtalk conference + skupinový facebook chat, facebook groups

facebook questions (group) + e-mail

facebook questions (group) + appointment slots + doodle + e-mail

facebook event, facebook groups event + srazy event + google cal event + lanyrd event + linkedin + e-mail

facebook message attachment + facebook group docs + writeboard + wiki + e-mail attachment + flickr + picasa + vimeo + youtube + ...

facebook friend + google contact + twitter contact + csv + vcard + linkedin contact

rtm + google tasks + e-mail

\section{Užitečnost jednotlivých funkcí}\label{utility}

viz \url{https://docs.google.com/spreadsheet/ccc?key=0AslWC4m8bQgYdHN3blFhcjNORk1hVmFlQmxkY05CQVE&pli=1#gid=0}

\section{Shrnutí}

bla bla, dopadlo to tak a tak, smysl má dělat toto, toto, toto, zbytek je na prd z různých důvodů...
